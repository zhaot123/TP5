...
//            ],
            //生成文件目录修改
            'file_dir' => [
                'controller' => $app_path .$module.'/controller/',
                'model'      => $app_path .$module.'/model/',
                'validate'   => $app_path .$module.'/validate/',
                'view'       => $app_path .$module.'/view/',
            ],
//            'file_dir' => [
//                'controller' => $app_path .$data['module'].'/controller/',
//                'model'      => $app_path .$data['module'].'/model/',
//                'validate'   => $app_path .$data['module'].'/validate/',
//                'view'       => $app_path .$data['module'].'/view/',
//            ],
        ];
    ...
    {
        $result = AdminMenuM::where('id', '<>', $current_id)->order('sort_id', 'asc')->order('id', 'asc')->column('id,parent_id,name,sort_id', 'id');
        foreach ($result as $r) {
    ...
    protected function createController()
    {
        ...
        //不生成控制器
        if ((int)$this->data['controller']['create'] == 0) {
            return true;
        }
...
//            file_put_contents($this->config['file_dir']['controller'] . $this->data['controller']['name'] . 'Controller' . '.php', $code);
            file_put_contents($this->config['file_dir']['controller'] . $this->data['controller']['name']. '.php', $code);
            $result = true;
        } catch (\Exception $e) {
            $msg    = $e->getMessage();
            $result = false;
    ...
    protected function createModel()
    {
        ...
        //不生成模型
        if ((int)$this->data['model']['create'] == 0) {
            return true;
        }
    ...
    {
        //不生成验证器
        if ((int)$this->data['validate']['create'] == 0) {
            return true;
        }
...
//            file_put_contents($this->config['file_dir']['validate'] . $this->data['validate']['name'] . 'Validate' . '.php', $code);
            file_put_contents($this->config['file_dir']['validate'] . $this->data['validate']['name'] . '.php', $code);
            $result = true;
        } catch (\Exception $e) {
            $msg    = $e->getMessage();
            $result = false;
    ...
    {
        //如果不需要列表视图，直接返回
        if ((int)$this->data['view']['create_index'] == 0) {
            return true;
        }
        ...
        $msg = '';
        try {
            //自动转换URL中的控制器和操作名true则转换目录名 小写字母+下划线+小写字母
            $controller_name = \think\facade\Config::get('app.url_convert') ? \think\Loader::parseName($this->data['controller']['name']) : strtolower($this->data['controller']['name']);
            file_put_contents($this->config['file_dir']['view'] . $controller_name . '/index.html', $code);
            $result = true;
        } catch (\Exception $e) {
            $msg    = $e->getMessage();
            $result = false;
    ...
    {
        //如果不需要列表视图，直接返回
        if ((int)$this->data['view']['create_add'] == 0) {
            return true;
        }
        ...
        $msg = '';
        try {
            if (!is_dir($this->config['file_dir']['view'] . strtolower($this->data['controller']['name']))) {//$this->data['table']
                $this->mkFolder($this->config['file_dir']['view'] . strtolower($this->data['controller']['name']));
            }

            file_put_contents($this->config['file_dir']['view'] . strtolower($this->data['controller']['name']) . '/add.html', $code);
            $result = true;
        } catch (\Exception $e) {
            $msg    = $e->getMessage();
            $result = false;
    ...
    protected function createMenu()
    {
        ...
        if ((int)$this->data['menu']['create'] < 0) {
            return true;
        }

        //菜单前缀
        $url_prefix = $this->data['module'] .'/'. strtolower($this->data['controller']['name']);
        //显示名称
        ...
        try {
            ...
            $result = AdminMenuM::create($parent);
