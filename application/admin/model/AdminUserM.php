 ...
 */
class AdminUserM extends Model
{
    use SoftDelete;
    ...
    {
        return $this->hasMany(AdminLogM::class, 'admin_user_id', 'id');
    }
    ...
    {
        return AdminRoleM::where('id', 'in', $data['role'])->column('id,name', 'id');
    }
    ...
    {
        $role_urls  = AdminRoleM::where('id', 'in', $data['role'])->where('status', 1)->column('url');
        $url_id_str = '';
        ...
        $auth_url = [];
        if (count($url_id) > 0) {
            $auth_url = AdminMenuM::where('id', 'in', $url_id)->column('url');
        }
        return $auth_url;
    }
    ...
    {
        if ($this->id === 1) {
            return AdminMenuM::where('is_show', 1)->order('sort_id', 'asc')->order('id', 'asc')->column('id,parent_id,name,url,icon,sort_id', 'id');
        }

        $role_urls = AdminRoleM::where('id', 'in', $this->role)->where('status', 1)->column('url');
        ...
        $url_id = array_unique(explode(',', $url_id_str));
        return AdminMenuM::where('id', 'in', $url_id)->where('is_show', 1)->order('sort_id', 'asc')->order('id', 'asc')->column('id,parent_id,name,url,icon,sort_id', 'id');
    }
