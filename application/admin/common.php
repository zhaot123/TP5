...
EOF;
        ...
        if ($data->auto_create_menu == 1) {
            ...
            $menu = AdminMenuM::get(function ($query) use ($url) {
                $query->where('url', $url);
            });
            if (!$menu) {
                $result = AdminMenuM::create([
                    'parent_id' => 43,
                    'name'      => $data->name,
                    'icon'      => $data->icon,
                    'is_show'   => 1,
