...
class AdminMenu extends Controller
{
    ...
    public function index(Request $request, AdminMenuM $model)
    {
        //查询所有菜单并以树的形式显示
        $result = $model->where("is_show",1)->order('sort_id asc, id asc')->column('*', 'id');
    ...
    public function add(Request $request, AdminMenuM $model, AdminMenuValidate $validate)
    {
        if ($request->isPost()) {
            $param           = $request->param();
    ...
    public function edit($id, Request $request, AdminMenuM $model, AdminMenuValidate $validate)
    {
    ...
    public function del($id, Request $request, AdminMenuM $model)
    {
        //判断是否有子菜单
