...
use app\currency\model\CurrencyM;
use app\reward\model\ContractM;
use app\reward\model\RewardM;
...
class Auth extends Controller
{
    ...
    //登录
    public function login(Request $request, AdminUserM $model, AdminUserValidate $validate)
    {
        if ($request->isPost()) {
            ...
            //登录成功如果不存在redis 配置项目存储redis
//            $redis = new Redis();



            //资金种类配置
//            $currency = CurrencyM::all();
//            foreach($currency as $k=>$v){
//                if(!$redis->get("currency:{$v['name']}")){
//                    $redis->set("currency:{$v['name']}",$v);
//                }
//            }
//            //推荐奖励配置
//            $reward = RewardM::find();
//            if(!$redis->get("reward:commission")){
//                $redis->set("reward:commission",$reward['commission']);
//            }
//            if(!$redis->get("reward:cost")){
//                $redis->set("reward:cost",$reward['cost']);
//            }
//            if(!$redis->get("reward:reward")){
//                $redis->set("reward:reward",$reward['reward']);
//            }
//            if(!$redis->get("reward:total")){
//                $redis->set("reward:total",$reward['total']);
//            }
//            $contract= ContractM::all();
//            foreach($contract as $ke=>$va){
//                if(!$redis->get("contract:{$va['id']}")){
//                    $redis->set("contract:{$va['id']}",$v);
//                }
//            }
//
